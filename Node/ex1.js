...
let biggestCountry = ""
let biggestPop = countries[0].population //took outside of loop

for (let c of countries) {
    //let biggestPop = countries[0].population

    if (c.population > biggestPop) {
        biggestPop = c.population //updating biggestPop
...
for (let c of countries) {
    let landmarks = c.landmarks || [{}] //make sure its iterable 

    for (let l of landmarks) {
        let tourismCount = l.tourismCount || 0 
        if (tourismCount > 1000) {
